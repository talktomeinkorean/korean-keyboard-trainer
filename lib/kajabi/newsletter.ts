 ...
 * 인증은 OAuth2 client_credentials 다. client_id·secret 은 Kajabi 대시보드의
 * Settings > Public API 에서 발급하고 Vercel 환경 변수에 넣는다 (Owner·Subowner 만 발급 가능).
 *
...
/**
 * 뉴스레터 폼에 이름·이메일을 넣는다.
 * 환경 변수가 없으면 아무것도 하지 않는다 ('skipped') — 로컬·프리뷰에서 실제 구독이 생기면 안 된다.
 */
