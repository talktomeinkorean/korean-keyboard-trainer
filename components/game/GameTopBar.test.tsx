...
describe('GameTopBar', () => {
  it('경과 시간을 mm:ss.cc 로 표시한다', () => {
    render(<GameTopBar elapsedMs={33120} muted={false} onToggleMuted={() => {}} />);
    expect(screen.getByTestId('race-timer')).toHaveTextContent('00:33.12');
  });

  it('소리 켜짐이면 스피커+음파 아이콘을 쓴다', () => {
    render(<GameTopBar elapsedMs={0} muted={false} onToggleMuted={() => {}} />);
  ...
  it('음소거면 전용 아이콘으로 바뀐다', () => {
    render(<GameTopBar elapsedMs={0} muted onToggleMuted={() => {}} />);
  ...
  it('사운드 버튼을 누르면 토글을 호출하고 상태를 알린다', () => {
    ...
    render(<GameTopBar elapsedMs={0} muted onToggleMuted={onToggle} />);
